...
class Saitoh:
    def __init__(self,wavelength,d,pitch):
        # everthing in micron!!!
        self.l_over_pitch = wavelength/pitch
        self.d_over_pitch = d/pitch
        self.aij = np.array([[0.54808, 0.71041, 0.16904, -1.52736],
                        [5.00401, 9.73491, 1.85765, 1.06745],
                        [-10.43248, 47.41496, 18.96849, 1.93229],
                        [8.22992, -437.50962, -42.4318, 3.89]]).transpose()

        self.bij = np.array([[5, 1.8, 1.7, -0.84],
                        [7, 7.32, 10, 1.02],
                        [9, 22.8, 14, 13.4]]).transpose()

        self.cij = np.array([[-0.0973, 0.53193, 0.24876, 5.29801],
                        [-16.70566, 6.70858, 2.72423, 0.05142],
                        [67.13845, 52.04855, 13.28649, -5.18302],
                        [-50.25518, -540.66947, -36.80372, 2.7641]]).transpose()
        self.dij = np.array([[7, 1.49, 3.85, -2],
                        [9, 6.58, 10, 0.41],
                        [10, 24.8, 15, 6]]).transpose()
    ...
    def V(self):
        ...
        return Ai_[0] + Ai_[1] / (1 + Ai_[2] * np.exp(Ai_[3] * self.l_over_pitch))
    ...
    def W(self):
        ...
        return Bi_[0] + Bi_[1] / (1 + Bi_[2] * np.exp(Bi_[3] * self.l_over_pitch))

    def nFSM(self):
        nFSM2 = self.n_co2 - 3 * Saitoh.V(self) ** 2 / (4 * np.pi ** 2) * self.l_over_pitch ** 2
        return np.sqrt(nFSM2)


    def neff(self):
        neff2 = self.n_co2 + 3 / (4 * np.pi ** 2) * self.l_over_pitch ** 2 * (Saitoh.W(self) ** 2 - Saitoh.V(self) ** 2)
