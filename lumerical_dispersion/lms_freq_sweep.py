...
output_dir = os.path.join(
    output_basedir, subfolder, date_str, description + "_" + date_time_str
)
...
mode = lumapi.MODE(
    filename=f"{subfolder}/PCF_20240422.lms", hide=True
)  # constructor for the Lumerical product


def set_initial_analysis_props(
    initial_frq_thz, d, pitch, fde_resolution, num_trial_modes
):
    mode.switchtolayout()
    mode.select("FDE")
