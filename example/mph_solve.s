...
#sweep definition
#K is number of modes to track and N is number of parameters to sweep over
K=6
N=5
P1_MIN=1.0
P1_MAX=1.4
...
export SUBMITDIR INPUTFILE P1_TAG P1 H5OUT

# ---- run MATLAB headless, connect with mphstart, run your script ----
START_TIME=$(date +%s)
cd /software/comsol/6.3/mli
matlab -nodesktop -nosplash -r "mphstart('localhost',${PORT}); run('${HOME}/input/mphRunner.m'); exit"
...
for i in {1..10}; do
  if [[ -s "$HOME/${H5OUT}" ]]; then
   echo "H5 successfully created: ${H5OUT}"
   END_TIME=$(date +%s)
