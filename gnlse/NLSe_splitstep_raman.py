# -*- coding: utf-8 -*-
"""
Created on Sat Nov 25 00:06:53 2023
...
class fiber_propagation:
    ...
    def run(self,shape):
        ...
        for i in range(self.z_steps):
            E[0:,i+1] = np.exp(0.5*nonlinear*(abs(E[0,i])**2-tr*(abs(E[1,i])**2-abs(E[-1,i])**2)/(2*self.deltau)))*E[0,i]
            E[-1:,i+1] = np.exp(0.5*nonlinear*(abs(E[1,i])**2-tr*(abs(E[0,i])**2-abs(E[-2,i])**2)/(2*self.deltau)))*E[1,i]
            E[1:-1,i+1] = np.exp(0.5*nonlinear*(abs(E[1:-1,i])**2-tr*(abs(E[2:,i])**2-abs(E[:-2,i])**2)/(2*self.deltau)))*E[1:-1,i]
            spectrum[:,i+1] = dispersion*fft.ifft(E[:,i+1])
            Ei = fft.fft(spectrum[:,i+1])
            E[0:,i+1] = np.exp(0.5*nonlinear*(abs(Ei[0])**2-tr*(abs(Ei[1])**2-abs(Ei[-1])**2)/(2*self.deltau)))*Ei[0]
            E[-1:,i+1] = np.exp(0.5*nonlinear*(abs(Ei[1])**2-tr*(abs(Ei[0])**2-abs(Ei[-2])**2)/(2*self.deltau)))*Ei[1]
            E[1:-1,i+1] = np.exp(0.5*nonlinear*(abs(Ei[1:-1])**2-tr*(abs(Ei[2:])**2-abs(Ei[:-2])**2)/(2*self.deltau)))*Ei[1:-1]
