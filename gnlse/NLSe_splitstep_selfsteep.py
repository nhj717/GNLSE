# -*- coding: utf-8 -*-
"""
Created on Sat Nov 25 00:52:04 2023
...
class fiber_propagation:
    ...
    "set initial values"    
    def __init__(self,alpha,beta2,beta3,s,tr,gamma,P0,T0):
        ...
        self.z_tot = 2.5*np.pi       #unit of LD
    ...
    def run(self,shape):
        ...
        for i in range(self.z_steps):
            E[0:,i+1] = np.exp(0.5*nonlinear*(abs(E[0,i])**2+(1j)*s*np.conjugate(E[0,i])*(E[1,i]-E[-1,i])/(2*self.deltau)+((1j)*s-tr)*(abs(E[1,i])**2-abs(E[-1,i])**2)/(2*self.deltau)))*E[0,i]
            E[-1:,i+1] = np.exp(0.5*nonlinear*(abs(E[1,i])**2+(1j)*s*np.conjugate(E[-1,i])*(E[0,i]-E[-2,i])/(2*self.deltau)+((1j)*s-tr)*(abs(E[0,i])**2-abs(E[-2,i])**2)/(2*self.deltau)))*E[1,i]
            E[1:-1,i+1] = np.exp(0.5*nonlinear*(abs(E[1:-1,i])**2+(1j)*s*np.conjugate(E[0,i])*(E[1,i]-E[-1,i])/(2*self.deltau)+((1j)*s-tr)*(abs(E[2:,i])**2-abs(E[:-2,i])**2)/(2*self.deltau)))*E[1:-1,i]
            spectrum[:,i+1] = dispersion*fft.ifft(E[:,i+1])
            Ei = fft.fft(spectrum[:,i+1])
            E[0:,i+1] = np.exp(0.5*nonlinear*(abs(Ei[0])**2+(1j)*s*np.conjugate(Ei[0])*(Ei[1]-Ei[-1])/(2*self.deltau)+((1j)*s-tr)*(abs(Ei[1])**2-abs(Ei[-1])**2)/(2*self.deltau)))*Ei[0]
            E[-1:,i+1] = np.exp(0.5*nonlinear*(abs(Ei[1])**2+(1j)*s*np.conjugate(Ei[-1])*(Ei[0]-Ei[-2])/(2*self.deltau)+((1j)*s-tr)*(abs(Ei[0])**2-abs(Ei[-2])**2)/(2*self.deltau)))*Ei[1]
            E[1:-1,i+1] = np.exp(0.5*nonlinear*(abs(Ei[1:-1])**2+(1j)*s*np.conjugate(Ei[1:-1])*(Ei[:-2]-Ei[2:])/(2*self.deltau)+((1j)*s-tr)*(abs(Ei[2:])**2-abs(Ei[:-2])**2)/(2*self.deltau)))*Ei[1:-1]
...
sim = fiber_propagation(alpha, beta2, beta3, s, tr, gamma, P0, T0)
sim.run("sech")
