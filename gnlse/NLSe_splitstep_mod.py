# -*- coding: utf-8 -*-
"""
Created on Wed Nov 22 23:17:18 2023
...
class fiber_propagation:
    ...
    "set initial values"    
    def __init__(self,alpha,beta2,gamma,P0,T0):
    ...
    def run(self,shape):
        ...
        dispersion = np.exp(0.5*(1j)*self.beta2*self.omega**2*self.delz) # phase factor
    ...
    def draw(self):
        ...
        pcm1 = ax1.pcolor(tt[:,index1:index2],zz[:,index1:index2],vis1[:,index1:index2],cmap = 'jet')
        ax1.set_title('Time domain')
        cb1 = plt.colorbar(pcm1,shrink = 0.75)
        
        pcm2 = ax2.pcolor(ff[:,index1:index2],zz_f[:,index1:index2],vis2[:,index1:index2],cmap = 'jet')
...
sim = fiber_propagation(alpha, beta2, gamma, P0, T0)
sim.run("sech")
